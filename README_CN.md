...
Halo 的透明窗口不会获取焦点，所有鼠标操作都会穿透到下面的应用。空闲
时窗口直接隐藏；Rust 使用系统文件事件监听，不做 500ms 轮询。
...
- 单个可取消的超时工作线程；
- 生产 JS 197.61 KB，gzip 62.22 KB。

在 2018 款 Intel Core i9 MacBook Pro、macOS 15.7.7 上，成品 App 的主进程
空闲 CPU 连续 6 次采样均为 0.0%，工作动画稳定在 1.6–1.7%，RSS 约
48–51 MiB。测试方法和验收边界见
...
v0.1 有意保持单显示器、未签名、纯本地。多显示器、签名/公证、
DMG/MSI 和更多主题属于后续版本。
